    ...
    >
      <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
        ...
        <Link href="/" className="flex items-center gap-2 group">
          <div className="w-8 h-8 rounded-full flex items-center justify-center" style={{ background: "linear-gradient(135deg, #2D5A35, #7A9E7E)" }}>
        ...
        <nav className="hidden md:flex items-center gap-8">
          {navLinks.map((link) => (
            <Link
              ...
              style={{ color: "#4A7A50" }}
              onMouseEnter={(e) => (e.currentTarget.style.color = "#2D5A35")}
      ...
      {/* Mobile Menu */}
      {menuOpen && (
        <div className="md:hidden glass border-t border-green-100 px-6 py-6 flex flex-col gap-4">
          {navLinks.map((link) => (
            <Link
              key={link.label}
              href={link.href}
